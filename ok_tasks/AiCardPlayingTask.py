...
from ok_tasks.MaterialCollectorTask import MaterialCollectorTask, infer_next_hero_slot  # 复用进场状态机并按新版间距推导第四武将卡槽。
...
def choose_hero_candidate(candidates, statistics, exploration_games=10, preferred=None):  # 从当前全部OCR候选中先执行用户偏好，再按安全可自动化的T0、T1顺序选将。
...
class AiCardPlayingTask(MaterialCollectorTask):  # 定义由训练模型决策的自动打牌任务。
    def __init__(self, *args, **kwargs):  # 初始化任务名称和 AI 专用配置。
        ...
        self.config_description.update({  # 为 GUI 配置补充中文说明。
            ...
            "Auto Select Hero": "识别当前三个或四个候选武将，优先在全被动技能角色中按统计选择；没有被动角色时再选择其他已拥有角色。",  # 说明新版四卡布局和旧版布局的安全回退。
    ...
    def _prepare_session(self):  # 初始化采集状态和训练模型会话。
        ...
        try:
            ...
            self.ai_model = TrainedModelAdapter(adapter_path, None).load()  # 内置规则模型不依赖外部权重文件。
    ...
    def _select_middle_hero(self):  # 识别当前三名或四名候选并优先在全被动技能武将中按统计选择。
        if not self.config.get("Auto Select Hero", True):  # 用户关闭智能选将时保留原固定中间位行为。
            return super()._select_middle_hero()  # 调用已验证的中间英雄选择流程。
        frame = self.next_frame()  # 获取稳定的选将画面用于当前全部卡槽OCR。
        candidates, slots = self._recognize_hero_candidates(frame)  # 首次识别当前三卡或四卡候选武将。
        if not any(hero in PASSIVE_OWNED_HEROES for hero in candidates):  # 当前候选没有全被动技能角色时才消耗换将次数。
            candidates, slots = self._swap_until_passive_candidate(candidates, slots)  # 按当前布局依次使用可确认的换将按钮寻找被动角色。
        ...
        if self._click_hero_confirm():  # 使用旧模板或新版OCR按钮确认当前武将。
            self.in_match = True  # 确认成功后将后续空白动画视为正常牌局等待。
        else:  # 新旧确认方式都失败时保留现场供下一轮诊断。
            self._capture_state("hero_confirm_missing", selected_frame, force=True)  # 保存按钮漂移现场，避免盲点固定坐标。
            self.log_warning("已选择武将，但未可靠识别到选定按钮，将在下一轮继续确认。")  # 明确告知未执行危险兜底点击。

    def _recognize_hero_candidates(self, frame):  # 同时探测旧三卡与新版四卡布局并选择识别结果更可靠的一组。
        layouts = []  # 保存每套布局的规范武将名和可点击卡槽。
        layout_specs = (("layout_a", tuple(f"hero_slot_{index}_layout_a" for index in (1, 2, 3))), ("layout_b", tuple(f"hero_slot_{index}_layout_b" for index in (1, 2, 3))))  # 定义已有标注中的两种基础布局。
        for layout_name, feature_names in layout_specs:  # 分别读取居中三卡和铺满四卡的前三个固定卡槽。
            if not all(self.feature_exists(name) for name in feature_names):  # 任一卡槽标注缺失时不使用不完整布局。
                continue  # 继续尝试另一套完整布局。
            slots = [self.get_box_by_name(name) for name in feature_names]  # 获取当前分辨率下的三个已标注卡片区域。
            if layout_name == "layout_b":  # 新版四卡布局的第四张与前三张保持等距。
                fourth_slot = infer_next_hero_slot(slots, frame.shape[1], frame.shape[0])  # 从真实标注间距推导最右侧卡片而不写死屏幕坐标。
                if fourth_slot is not None:  # 只有推导区域完全位于画面内时才加入候选。
                    slots.append(fourth_slot)  # 将第四卡槽纳入OCR、选择和换将流程。
            candidates = []  # 保存当前布局按屏幕顺序识别的规范武将名。
            for slot in slots:  # 对当前布局的每张完整卡片独立执行OCR。
                boxes = self.ocr(box=slot, frame=frame, threshold=0.18, target_height=1080, log=False)  # 在单卡区域识别底部武将名及其可能别名。
                text = "".join(str(box.name or "") for box in sorted(boxes, key=lambda box: (box.y, box.x)))  # 合并同一卡槽内的横排或竖排文字。
                candidates.append(normalize_hero_name(text))  # 只接受账号支持名单中的完整规范武将名。
            layouts.append((layout_name, candidates, slots))  # 保存该布局供可靠识别数量比较。
        if not layouts:  # 素材完全缺失时不能推导任何安全卡槽。
            self.hero_slot_layout = None  # 清除布局状态防止换将使用旧坐标。
            return [None, None, None], [None, None, None]  # 保持旧调用至少三个位置的兼容结果。
        layout_name, candidates, slots = max(layouts, key=lambda item: (sum(hero is not None for hero in item[1]), item[0] == "layout_b"))  # 优先完整名称更多的布局，同分时采用当前新版四卡布局。
        self.hero_slot_layout = layout_name  # 保存真实布局供换将按钮选择使用。
        return candidates, slots  # 返回保持屏幕顺序对应的候选与卡槽。

    def _swap_until_passive_candidate(self, candidates, slots):  # 使用当前布局真实换将按钮直到出现全被动技能角色或完成一轮尝试。
        if getattr(self, "hero_slot_layout", None) == "layout_b":  # 新版四卡布局为每张卡提供独立换将按钮。
            swap_targets = [(index, self._hero_swap_box(slot)) for index, slot in enumerate(slots) if slot is not None]  # 从卡片右下角派生四个按钮区域。
        else:  # 旧三卡布局继续使用已经标注并验证过的两个按钮模板。
            swap_targets = [(index, self._find_first_feature([feature_name], self.config.get("Template Threshold", 0.8))) for index, feature_name in ((0, "Hero Slot 1 - Swap Player"), (1, "Hero Slot 2 - Swap Player"))]  # 获取旧布局当前帧实际存在的按钮。
        for slot_index, swap_button in swap_targets:  # 每个真实按钮最多点击一次，防止同一选将界面无限消耗换将次数。
    ...
    @staticmethod
    def _hero_swap_box(slot):  # 从新版完整武将卡片推导其右下角换将按钮区域。
        return Box(int(slot.x + slot.width * 0.82), int(slot.y + slot.height * 0.82), max(1, int(slot.width * 0.28)), max(1, int(slot.height * 0.26)), name="hero_swap_layout_b")  # 让点击中心落在截图中圆形换将图标中央。
