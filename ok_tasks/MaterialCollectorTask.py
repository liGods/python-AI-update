...
class MaterialCollectorTask(BaseTask):  # 定义用户手动启动的自动素材采集任务。
    ...
    def _select_middle_hero(self):  # 固定选择三个候选武将中的中间武将。
        slot_name = "hero_slot_2_layout_a"  # 使用未选中布局的第二个武将槽位标注。
        if self.feature_exists(slot_name):  # 检查中间武将槽位标注是否可用。
            middle_slot = self.get_box_by_name(slot_name)  # 读取中间武将卡片的固定区域。
        ...
        if self._click_feature("select", after_sleep=1.5):  # 点击选定按钮进入牌局并确认动作已经执行。
